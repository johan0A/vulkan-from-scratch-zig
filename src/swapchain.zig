...
pub const Swapchain = struct {
    ...
    pub fn initRecycle(gc: *const GraphicsContext, allocator: Allocator, extent: vk.Extent2D, old_handle: vk.SwapchainKHR) !Swapchain {
        const caps = try gc.instance_dispatch.getPhysicalDeviceSurfaceCapabilitiesKHR(gc.phys_device, gc.surface);
        ...
        errdefer gc.device_dispatch.destroySwapchainKHR(gc.device, handle, null);

        if (old_handle != .null_handle) {
            // Apparently, the old swapchain handle still needs to be destroyed after recreating.
            gc.device_dispatch.destroySwapchainKHR(gc.device, old_handle, null);
        }
    ...
    pub fn deinit(self: Swapchain) void {
        ...
        self.gc.device_dispatch.destroySwapchainKHR(self.gc.device, self.handle, null);
    }
    ...
    pub fn present(self: *Swapchain, cmdbuf: vk.CommandBuffer) !PresentState {
        ...
        try self.gc.device_dispatch.resetFences(self.gc.device, 1, @ptrCast(&current.frame_fence));

        // Step 2: Submit the command buffer
        const wait_stage = [_]vk.PipelineStageFlags{.{ .top_of_pipe_bit = true }};
        try self.gc.device_dispatch.queueSubmit(self.gc.graphics_queue.handle, 1, &[_]vk.SubmitInfo{.{
        ...
        const result = try self.gc.device_dispatch.acquireNextImageKHR(
            self.gc.device,
            self.handle,
            std.math.maxInt(u64),
...
const SwapImage = struct {
    ...
    fn init(gc: *const GraphicsContext, image: vk.Image, format: vk.Format) !SwapImage {
        ...
        errdefer gc.device_dispatch.destroyFence(gc.device, frame_fence, null);
    ...
    fn deinit(self: SwapImage, gc: *const GraphicsContext) void {
        ...
        gc.device_dispatch.destroyImageView(gc.device, self.view, null);
        gc.device_dispatch.destroySemaphore(gc.device, self.image_acquired, null);
        gc.device_dispatch.destroySemaphore(gc.device, self.render_finished, null);
        gc.device_dispatch.destroyFence(gc.device, self.frame_fence, null);
    }

    fn waitForFence(self: SwapImage, gc: *const GraphicsContext) !void {
        _ = try gc.device_dispatch.waitForFences(gc.device, 1, @ptrCast(&self.frame_fence), vk.TRUE, std.math.maxInt(u64));
    }
};
...
fn initSwapchainImages(gc: *const GraphicsContext, swapchain: vk.SwapchainKHR, format: vk.Format, allocator: Allocator) ![]SwapImage {
    ...
    _ = try gc.device_dispatch.getSwapchainImagesKHR(gc.device, swapchain, &count, null);
    const images = try allocator.alloc(vk.Image, count);
    defer allocator.free(images);
    _ = try gc.device_dispatch.getSwapchainImagesKHR(gc.device, swapchain, &count, images.ptr);
...
fn findSurfaceFormat(gc: *const GraphicsContext, allocator: Allocator) !vk.SurfaceFormatKHR {
    ...
    var count: u32 = undefined;
    _ = try gc.instance_dispatch.getPhysicalDeviceSurfaceFormatsKHR(gc.phys_device, gc.surface, &count, null);
    const surface_formats = try allocator.alloc(vk.SurfaceFormatKHR, count);
    defer allocator.free(surface_formats);
    _ = try gc.instance_dispatch.getPhysicalDeviceSurfaceFormatsKHR(gc.phys_device, gc.surface, &count, surface_formats.ptr);
...
fn findPresentMode(gc: *const GraphicsContext, allocator: Allocator) !vk.PresentModeKHR {
    var count: u32 = undefined;
    _ = try gc.instance_dispatch.getPhysicalDeviceSurfacePresentModesKHR(gc.phys_device, gc.surface, &count, null);
    const present_modes = try allocator.alloc(vk.PresentModeKHR, count);
    defer allocator.free(present_modes);
    _ = try gc.instance_dispatch.getPhysicalDeviceSurfacePresentModesKHR(gc.phys_device, gc.surface, &count, present_modes.ptr);
